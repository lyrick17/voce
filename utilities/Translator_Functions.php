...
class Translator
{
    ...
    static function translate($input = '', $src = '', $target = '', $filename)
    {
        ...
        if ($response == "~<b>Voce Connection Error</b>: Please connect to the Internet to continue translating~") {
            ErrorHandling::audioError3($filename); // pass the filename so it can be deleted, since it's not processed
        }
        return $response;
    }

    static function translate_t2t($input = '', $src = '', $target = '')
    {
